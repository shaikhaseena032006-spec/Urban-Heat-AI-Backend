    ...
    if response.status_code == 200:
        ...
        with MemoryFile(response.content) as memfile:
            with memfile.open() as dataset:
                ...
                avg_value = float(np.mean(band))
                min_value = float(np.min(band))
                max_value = float(np.max(band))
...
@app.get("/api/city-analytics/{city}")
def city_analytics(city: str):

    ndvi = get_satellite_index(city, "ndvi")
    ndwi = get_satellite_index(city, "ndwi")
    ndbi = get_satellite_index(city, "ndbi")
...
def get_city_ndvi(city: str):
    ...
    return get_satellite_index(city, "ndvi")
...
def get_ndwi(city: str):
    return get_satellite_index(city, "ndwi")
 
 
@app.get("/ndbi/{city}")
def get_ndbi(city: str):
    return get_satellite_index(city, "ndbi")
 
 
# ======================
# Static City Overview Data
# ======================
def image_score(filename):
    ...
    try:
        ...
        return round(float(arr.mean()) / 255, 2)
...
def city_data(city: str):
    ...
    ndwi_file = f"ndwi_{city}.png"
    ndbi_file = f"ndbi_{city}.png"
    ndvi_file = f"ndvi_{city}.png"
 
    vegetation_score = image_score(ndvi_file)
    water_score = image_score(ndwi_file)
    builtup_score = image_score(ndbi_file)
